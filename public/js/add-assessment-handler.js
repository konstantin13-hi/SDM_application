document.addEventListener('DOMContentLoaded', function() {
    ...
    function addAssessmentToDOM(assessment) {
        console.log('Adding assessment to DOM:', assessment);
        ...
        listItem.innerHTML = `
            ...
            <input type="number" class="weight-input" value="${Math.round(assessment.weight)}" disabled>
            <div>
    ...
    async function validateTotalWeight(newWeight = 0, assessmentId = null) {
        let url = `http://localhost:3000/get-total-weight?courseId=${courseId}`;
        if (assessmentId) {
            url += `&assessmentId=${assessmentId}`;
        }
    
        console.log(`Fetching total weight with URL: ${url}`);
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Failed to validate total weight');
            }
            const data = await response.json();
            console.log(`Total weight received: ${data.totalWeight}`);
            return data.totalWeight || 0;
        } catch (error) {
            console.error('Error validating total weight:', error);
            displayMessage('Error validating total weight.', 'danger');
            return 0; // Opcjonalnie, można zdecydować o innym zachowaniu
        }
    }
